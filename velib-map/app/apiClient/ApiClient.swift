...
extension ApiClient {
  static let live = Self(
    fetchStation: { id in
      ...
      guard let station = try responseEnvelope.records.map({ $0.station }).first?.toStation() else {
        throw "No station"
      }
    ...
    fetchAllStations: {
      ...
      let (data, _) = try await URLSession.shared.data(from: url)
      let responseEnvelope = try JSONDecoder().decode(FetchStationObjectResponseRoot.self, from: data)
      let stations = try responseEnvelope.records.map({ try $0.station.toStation()})
